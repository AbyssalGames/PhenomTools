...
public class DynamicSoundGroupInspector : Editor {
    ...
    {
        Dictionary<DynamicGroupVariation, float> clips = new Dictionary<DynamicGroupVariation, float>();
        ...
        foreach (DynamicGroupVariation variant in variations)
        {
            AudioClip clip = null;

            switch (variant.audLocation)
            {
                case MasterAudio.AudioLocation.Clip:
                    if (variant.VarAudio.clip == null)
                    {
                        continue;
                    }
                    clip = variant.VarAudio.clip;
                    break;
                case MasterAudio.AudioLocation.FileOnInternet:
                    continue;
                case MasterAudio.AudioLocation.ResourceFile:
                    if (string.IsNullOrEmpty(variant.resourceFileName))
                    {
                        continue;
                    }

                    clip = Resources.Load(variant.resourceFileName) as AudioClip;
            ...
            if (!AudioUtil.IsClipReadyToPlay(clip))
            {
                Debug.Log("Clip is not ready to play (streaming?). Skipping '" + variant.name + "'.");
                continue;
            }
            ...
            Debug.Log("Measuring amplitude of '" + clip.name + "'.");

            clip.GetData(buffer, 0);

            for (int c = 0; c < clip.samples; c++)
            {
                average += Mathf.Pow(buffer[c], 2);
            }

            average = Mathf.Sqrt(1f / clip.samples * average);
        ...
        foreach (KeyValuePair<DynamicGroupVariation, float> kv in clips)
        {
            if (kv.Value == 0)
            {
                // skip
